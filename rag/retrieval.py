...
def build_bm25_index(chunks: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if not corpus:
        return {"bm25": None, "id_map": [], "chunk_by_id": {}}
...
def dense_search(
    ...
    # Default (unnamed) vector collection — no `using` parameter needed
...
def hybrid_search(
    ...
    fused_scores = rrf_fuse(dense_hits=dense_hits, bm25_hits=bm25_hits, rrf_k=rrf_k)

    ranked_ids = sorted(fused_scores.items(), key=lambda item: item[1], reverse=True)[:k]
